...
const PriorityWindowSchema = z.object({
  type: z.literal("PriorityWindow"),
  playerId: PlayerIdSchema,
});
const FocusWindowSchema = z.object({
  type: z.literal("FocusWindow"),
  playerId: PlayerIdSchema,
  battlefieldId: BattlefieldIdSchema,
});
const ChooseTargetsDecisionSchema = z.object({
  type: z.literal("ChooseTargets"),
  playerId: PlayerIdSchema,
...
const ChooseYesNoDecisionSchema = z.object({
  type: z.literal("ChooseYesNo"),
  playerId: PlayerIdSchema,
  decisionId: DecisionIdSchema,
  prompt: z.string(),
});
const ChooseOneDecisionSchema = z.object({
  type: z.literal("ChooseOne"),
  playerId: PlayerIdSchema,
  decisionId: DecisionIdSchema,
  options: z.array(z.string()),
});
const AssignDamageDecisionSchema = z.object({
  type: z.literal("AssignDamage"),
  playerId: PlayerIdSchema,
  decisionId: DecisionIdSchema,
  attackers: z.array(CombatUnitSchema),
  totalDamage: z.number().int().nonnegative(),
});
const ChooseBattlefieldDecisionSchema = z.object({
  type: z.literal("ChooseBattlefield"),
  playerId: PlayerIdSchema,
  options: z.array(CardIdSchema),
});
const ChooseMulliganDecisionSchema = z.object({
  type: z.literal("ChooseMulligan"),
  playerId: PlayerIdSchema,
